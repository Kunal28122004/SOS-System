...
3.  **Set up environment variables:**
    Create a file named `.env.local` in the root of your project. Copy the contents of `.env.example` into it and fill in the values from your Firebase project.
...
1.  **Admin Login**: Navigate to the homepage, select the "Admin" tab. Enter an admin name and a unique Admin ID (e.g., `city-pd`). This will register the admin and log you into the admin dashboard.
2.  **User Login**: Open a new browser or incognito window. On the homepage, select the "User" tab. Enter your name, phone number, and the Admin ID you created in the previous step. This will log you into the user dashboard.
3.  **Trigger an SOS**: As the user, press the large "SOS" button. You can optionally add a custom message.
4.  **Monitor Alerts**: As the admin, you will see the SOS alert appear on your dashboard in real-time, along with the user's location and message. The user's card in the "Tracked Users" section will also be highlighted.
